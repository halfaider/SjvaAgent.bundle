...
"""
2026.03.21
  - 업데이트 용 토큰은 아래 순서에 따라 먼저 발견되는 토큰을 사용

    - Context (자동): 에이전트 요청에 담긴 로컬 토큰
    - 에이전트 설정 (수동): 사용자가 에이전트 설정에 입력한 토큰
    - Preferences.xml (자동): 설정 파일에 저장된 온라인 토큰 (리눅스 한정)
  
  - 특정 시즌만 선택해서 업데이트할 수 있는 설정 추가
    
    예시) {"12345": [202001, 202101], "67890": []}
      메타데이터 아이템 12345은 시즌 202001, 202101만 업데이트하고,
      메타데이터 아이템 67890은 시즌 업데이트(에피소드 포함)를 하지 않음
  
  - show.yaml을 보조 메타데이터로 사용하지 "않을" 섹션 목록 설정 추가
  - 메타데이터 새로고침시 번들 폴더 삭제 설정 추가
  - 시즌 경로에 Daum ID가 있을 경우 활용
    
    예시) 시즌 1001 미스터트롯의 맛 (2020) {daum-88030}
  ...
  - 시즌 번호 체계는 다음과 같이 처리
       ...
       100 ~   999: 재생 버전별 시즌
      1000 ~  9999: 사용자 정의 시즌
     10000 ~ 99999: 사용자 정의 시즌
    100000 ~      : 시즌 1로 취급

2026.03.15
  - 고아 파일 문제 수정
  - ktv 코드에 따라 메타데이터 적용 순서 변경
  - TMDB 폴더도 매칭 선택 가능하게 변경
  - 시즌 메타데이터 업데이트를 비동기로 실행
  - 각 시즌별 포스터를 중복 다운로드하는 문제 수정
  - 가능할 경우 WEBP를 JPEG 혹은 PNG로 변경
