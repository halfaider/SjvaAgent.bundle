...
try:
    ...
    logger = logging.getLogger('sjva_show_normal')
    logger.setLevel(logging.DEBUG) 
    formatter = logging.Formatter(u'[%(asctime)s|%(levelname)s|%(filename)s|%(lineno)d] : %(message)s')
    file_max_bytes = 10 * 1024 * 1024 
    filename = os.path.join(os.path.dirname( os.path.abspath( __file__ ) ), '../../', 'Logs', 'sjva.scanner.show_folder.log')
...
episode_regexps = [
    r'\.?([sS](?P<season>[0-9]{1,2}))?[eE](?P<ep>[0-9]{1,5})',
    ur'(?P<ep>\d{1,4})[회화]',
    r'^(?P<ep>\d{1,4})[^\d]+'
]
        ...
        for idx, f in enumerate(files):
            ...
            try:
                ...
                if tempDone == False:
                    ...
                    tv_show.parts.append(f)
                    mediaList.append(tv_show)
                    logger.debug('  - APPEND2 by episode: %s' % tv_show)
                    tempDone = True
